...
const tab1Marker = "{adminActiveTab === 'users' && (";
const tab1Index = content.indexOf(tab1Marker);

const tab2Marker = "{adminActiveTab === 'inventory' && (";
// Find the SECOND occurrence of this marker (the first is the refresh button)
const tab2Index = content.indexOf(tab2Marker, content.indexOf(tab2Marker) + 1);

let usersContent = content.substring(tab1Index, tab2Index).trim();

const adminEndBlock = `            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>`;
const inventoryEndIndex = content.lastIndexOf("</motion.div>\n          </motion.div>");
let inventoryContent = content.substring(tab2Index, inventoryEndIndex).trim();
...
let supportContent = `
              {adminActiveTab === 'support' && (
                <div className="bg-[#161c2e] border border-slate-800 rounded-2xl p-8 flex flex-col items-center justify-center min-h-[400px] text-center">
                  <div className="w-16 h-16 bg-[#0a0d14] border border-slate-800 rounded-full flex items-center justify-center text-slate-500 mb-4">
...
let logsContent = `
              {adminActiveTab === 'logs' && (
                <div className="bg-[#161c2e] border border-slate-800 rounded-2xl p-8 flex flex-col items-center justify-center min-h-[400px] text-center">
                  ...
                  <p className="text-sm text-slate-400 max-w-md">سجلات المبيعات والتعويضات للبطاقات ستظهر هنا قريباً.</p>
                </div>
              )}
`;
            ...
            <div className="w-20 md:w-64 bg-[#0a0d14] border-l border-slate-800 flex flex-col flex-shrink-0 transition-all duration-300 relative z-20">
              ...
              <div className="flex-1 overflow-y-auto py-4 px-2 md:px-4 space-y-2 custom-scrollbar">
                ...
                >
                  ...
                  <span className="hidden md:block font-medium text-sm">سجل المبيعات</span>
                </button>
              </div>
...
fs.writeFileSync('/tmp/new_admin_section_v2.txt', newContent);

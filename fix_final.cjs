...
const t1 = content.indexOf("              {/* TAB 1: USER MANAGEMENT & CARD COMPENSATION */}");
const t2 = content.indexOf("              {/* TAB 2: INVENTORY & UPLOAD */}");
const tEnd = content.lastIndexOf("            </motion.div>\n          </motion.div>");

let usersContent = content.substring(t1, t2).trim();
let inventoryContent = content.substring(t2, tEnd).trim();
...
// I need to find the OLD block to replace.
// Start: "      {/* Admin Dashboard Modal */}"
// End: "      </AnimatePresence>" immediately after "showAdminDashboard && ("

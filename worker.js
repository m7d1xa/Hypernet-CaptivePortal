 ...
 */
export function escapeMikrotikString(val) {
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (pathname === "/api/auth/login" || pathname === "/api/login") {
        ...
        if (card) {
          // STRICT: Do NOT mutate status to 'USED'. Retain card's real status (AVAILABLE, SOLD, ACTIVE, EXPIRED)
          ...
          return jsonResponse({
            ...
            card: {
              ...
              activationTime: card.activated_at || null,
      ...
      if (url.pathname === "/api/cards/buy" && request.method === "POST") {
        ...
        const soldCard = await env.DB.prepare(
          "SELECT id, username, password, package_name, status FROM cards WHERE owner_id = ? AND status = 'مباع' ORDER BY id DESC LIMIT 1"
        ).bind(user_id).first();
      ...
      if (pathname === "/api/cards/user" || pathname === "/api/user/cards") {
        ...
        if (username) {
          query += "AND (LOWER(username) = ? OR assigned_user_id IN (SELECT id FROM users WHERE LOWER(username) = ?)) ";
      ...
      if (pathname.startsWith("/api/admin/")) {
        ...
        if (pathname === "/api/admin/cards/bulk" || pathname === "/api/cards/bulk") {
          ...
          const body = await request.json().catch(() => ({}));
          const cardsArray = Array.isArray(body) ? body : Array.isArray(body.cards) ? body.cards : [];
        ...
        if (pathname === "/api/admin/cards") {
          ...
          const whereClauses = ["LOWER(card_number) NOT IN ('username', 'password', 'package', 'اسم المستخدم')"];
          const params = [];
